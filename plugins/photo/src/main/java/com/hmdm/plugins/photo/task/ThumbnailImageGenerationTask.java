...
public class ThumbnailImageGenerationTask implements Runnable {
     ...
     */
    public ThumbnailImageGenerationTask(Photo photo, String baseFilesDir, PhotoDAO photoDAO) {
    ...
    public void run() {
        ...
        try {
            ...
            java.nio.file.Path uploadFilePathDir = Paths.get(this.baseFilesDir, "plugin", "photo", "thumbnail", yearDir, monthDir, dayDir);
            uploadFilePathDir = Files.createDirectories(uploadFilePathDir);

            String localFileName = UUID.randomUUID().toString() + ".png";
